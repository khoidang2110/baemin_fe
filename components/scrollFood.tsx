        ...
        <div className="bg-white rounded-2xl w-full" style={{ height: '300px' }}>
            <div className="w-full h-full flex flex-col px-4 pt-4 pb-2" style={{ height: '300px' }}>
                ...
                <div className="w-full relative h-full">
                    {currentIndex > 0 && (
                        <button onClick={handlePrev} className="absolute hover:text-beamin hover:bg-slate-50 bg-white top-20 w-8 h-8 rounded-full z-20">
                            <LeftOutlined />
                        </button>
                    )}
                    <div ref={containerRef} className="scroll-container w-full h-full flex flex-row gap-3">
                        {items.items.map((item, index) => (
                            <div key={index} onClick={handleNavigate} className="group w-48 h-full cursor-pointer">
                    ...
                    {currentIndex < items.items.length - 1 && (
                        <button onClick={handleNext} className="absolute hover:text-beamin hover:bg-slate-50 bg-white top-20 right-1 w-8 h-8 rounded-full z-20">
                            <RightOutlined />
                        </button>
                    )}
                </div>
